...
const positions = {
  TOP: isIOS
    ? isIphoneX
      ? 44 + TOAST_CLEARANCE
      : 20 + TOAST_CLEARANCE
    : StatusBar.currentHeight + TOAST_CLEARANCE,
  BOTTOM: -20,
  CENTER: 0
};
...
class ToastContainer extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      positionY: new Animated.Value(-SLIDE_DISTANCE),
  ...
  render() {
    ...
    let position = offset
      ? {
          [offset < 0 ? "bottom" : "top"]:
            offset < 0 ? this._keyboardHeight - offset - SLIDE_DISTANCE : offset - SLIDE_DISTANCE
        }
      : {
          top: 0,
          bottom: this._keyboardHeight
        };
    const translateY = this.state.positionY.interpolate({
      inputRange: [0, SLIDE_DISTANCE],
      outputRange: offset < 0 ? [SLIDE_DISTANCE, 0] : [-SLIDE_DISTANCE, 0]
    });
...
export { positions, durations, slideDirections };
